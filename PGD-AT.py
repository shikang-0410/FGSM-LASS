...
from utils import std, get_loaders, evaluate_pgd, evaluate_standard, label_smoothing, LabelSmoothLoss, \
    attack_pgd, get_loaders_cifar100
...
def get_args():
    parser = argparse.ArgumentParser('PGD-AT')
    ...
    parser.add_argument('--out_dir', default='PGD_AT_output', type=str, help='Output directory')
...
output_path = os.path.join(output_path, 'iters_' + str(args.iters))
...
if args.target_model == "ResNet18":
    target_model = resnet18(10 if args.dataset == 'cifar10' else 100)
else:
    target_model = WideResNet(num_classes=10 if args.dataset == 'cifar10' else 100)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        delta = attack_pgd(target_model, data, target, epsilon, alpha, args.iters, 1)
        output = target_model(data + delta)
        if args.factor < 1:
            smooth_label = Variable(
                torch.tensor(label_smoothing(target, args.factor, 10 if args.dataset == 'cifar10' else 100)).cuda())
