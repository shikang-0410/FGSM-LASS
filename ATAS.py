...
def get_args():
    parser = argparse.ArgumentParser('ATAS')
    ...
    parser.add_argument('--out_dir', default='ATAS_output', type=str, help='Output directory')
...
output_path = os.path.join(output_path, 'beta_' + str(args.beta))
...
v = torch.zeros(50000).cuda()
init_delta = torch.zeros(50000, 3, 64, 64).cuda()
gamma = args.c * 16. / 255
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        idx = range(start_idx, start_idx + data.shape[0])
        ...
        if epoch == 1:
            ...
            delta = init_delta[idx].detach()

        delta.requires_grad = True
        output_adv = target_model(data + delta)
        grad = torch.autograd.grad(F.cross_entropy(output_adv, target), [delta])[0]
        delta.requires_grad = False
        norm_ = torch.norm(grad.reshape(grad.shape[0], -1), dim=1)

        v[idx] = args.beta * v[idx] + (1 - args.beta) * norm_ ** 2
        alpha = gamma / (args.c + torch.sqrt(v[idx]))
        delta = clamp(delta + alpha[:, None, None, None] * torch.sign(grad), -epsilon, epsilon)
        delta.data = clamp(delta, lower_limit - data, upper_limit - data)

        init_delta[idx] = delta.detach()
