...
from utils import std, get_loaders, evaluate_pgd, evaluate_standard, label_smoothing, LabelSmoothLoss, attack_fgsm, \
    grad_align_loss, get_loaders_cifar100, get_loaders_tiny_imagenet
...
def get_args():
    parser = argparse.ArgumentParser('FGSM-GA')
    ...
    parser.add_argument('--out_dir', default='FGSM_GA_output', type=str, help='Output directory')
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss += grad_align_loss(target_model, data, target, eps=8 / 255., lamd=0.2)
